...
export default function GalleryGrid({ items, onItemClick }: GalleryGridProps) {
  const sortedItems = items.sort((a, b) => new Date(b.timeCreated).getTime() - new Date(a.timeCreated).getTime());
  return (
        ...
        >
          {item.name.includes(".png") || item.name.includes(".jpg") || item.name.includes(".jpeg") || item.name.includes(".JPG") ? (
            ...
            <div className="relative w-full h-48 bg-gray-200 rounded-lg flex items-center justify-center">
              <div className="p-2 rounded-full flex items-center justify-center bg-muted-foreground border border-black"><Play size={32} className="text-white" /></div>
            
              <video src={item.url} className="w-full h-full object-cover rounded-lg absolute inset-0 opacity-50" />
            </div>
          )}
          <span className="p-1 rounded-md absolute bg-muted-foreground text-white top-3 text-xs right-3">  {formatSize(item.size)}</span>
